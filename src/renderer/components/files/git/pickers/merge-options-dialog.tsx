  ...
  return (
    <>
      ...
      <form className="mt-4 flex flex-col gap-3" onSubmit={onSubmit}>
          ...
          {MERGE_OPTIONS.map((item) => {
            ...
            return (
              <label
                key={item.option}
                htmlFor={inputId}
                className="flex cursor-pointer items-start gap-3 rounded-md border border-mist-border bg-frosted-veil px-3 py-2 text-app-ui-sm"
              >
                <input
                  id={inputId}
                  type="radio"
        ...
        {option === "squash" ? (
          <p className="rounded-md border border-mist-border bg-frosted-veil px-3 py-2 text-app-ui-sm text-muted-foreground">
