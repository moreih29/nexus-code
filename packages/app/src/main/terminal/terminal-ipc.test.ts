...
} from "../workspace/shell/workspace-terminal-registry";
import { TerminalBridge, type TerminalBridgeTransport } from "../../renderer/terminal/terminal-bridge";
import {
...
describe("TerminalMainIpcRouter + TerminalBridge", () => {
  ...
  test("terminal IPC wiring remains sidecar/harness independent", async () => {
    ...
    const rendererSource = await readFile(
      path.join(mainDirectory, "../../renderer/terminal/terminal-bridge.ts"),
      "utf8",
    );
