...
export interface LspProcessStream {
...
export interface LspChildProcess extends EventEmitter {
  stdout: LspProcessStream | null;
  stderr: LspProcessStream | null;
  stdin: LspProcessInput | null;
  pid?: number;
  killed?: boolean;
...
const LSP_LANGUAGES: readonly LspLanguage[] = ["typescript", "python", "go"];
...
export class LspService {
  ...
  private readonly spawnProcess: LspSpawnProcess;
  ...
  private async destroyProcess(child: LspChildProcess): Promise<void> {
    try {
      child.stdin?.end?.();
    } catch {
      child.stdin?.destroy?.();
...
function defaultSpawnProcess(
  ...
  return spawnDefault(command, [...args], options) as LspChildProcess;
}
...
function createFailedProcessStub(error: unknown): LspChildProcess {
  const child = new EventEmitter() as LspChildProcess;
  child.stdout = null;
  child.stderr = null;
  child.stdin = null;
  child.killed = true;
...
function severityFromLsp(severity: number | undefined): LspDiagnosticSeverity {
  switch (severity) {
    case 1:
      return "error";
    case 2:
...
function languageIdFor(language: LspLanguage): string {
  return language;
}
