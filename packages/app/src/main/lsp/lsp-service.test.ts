...
describe("LspService", () => {
  ...
  test("uses stdio JSON-RPC framing and propagates diagnostics by workspace-relative path", async () => {
    ...
    const spawnProcess: LspSpawnProcess = (command, args, options) => {
  ...
  test("falls back from `gopls serve` to bare `gopls` when serve exits before initialize", async () => {
    ...
    const spawnProcess: LspSpawnProcess = (command, args) => {
...
class FakeLanguageServerProcess extends EventEmitter implements LspChildProcess {
...
class ExitingLanguageServerProcess extends EventEmitter implements LspChildProcess {
  public readonly stdout = new PassThrough();
  public readonly stderr = new PassThrough();
  public readonly killCalls: NodeJS.Signals[] = [];
  public readonly stdin: LspProcessInput;
