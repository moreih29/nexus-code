...
import { OpenSessionSidecarLifecycleManager } from "../sidecar/sidecar-lifecycle-manager";
import {
  SidecarBridge,
...
import { ClaudeSettingsConsentStore, ClaudeSettingsManager } from "../claude/claude-settings-manager";
import {
  ClaudeSettingsRegistrationCoordinator,
  RendererClaudeSettingsConsentRequester,
} from "../claude/claude-settings-registration";
import { CodexSettingsConsentStore, CodexSettingsManager } from "../codex/codex-settings-manager";
import { CodexSettingsRegistrationCoordinator } from "../codex/codex-settings-registration";
import { OpenCodeSseObserverService } from "../opencode/opencode-sse-observer-service";
import {
  buildOpenCodeTerminalEnvOverrides,
  ensureOpenCodeWorkspaceShims,
} from "../opencode/opencode-runtime";
import { ShellEnvironmentResolver } from "../shell-environment/shell-environment-resolver";
import { ClaudeSessionTranscriptService } from "../claude/claude-session-transcript-service";
import { registerObserverSurfacesIpcHandlers, type ObserverSurfacesIpcHandlers } from "../observer/observer-surfaces-ipc";
import { WorkspaceFilesService } from "../workspace/files/workspace-files-service";
import { registerEditorBridgeIpcHandlers, type EditorBridgeIpcHandlers } from "../editor/editor-bridge-ipc";
...
} from "../workspace/persistence/workspace-keyboard-shortcut-bridge";
...
export async function composeElectronAppServices(
  ...
  const workspaceShellService = new WorkspaceShellService(
    ...
    sidecarLifecycleManager,
    {
      stopTerminalsForClosedWorkspace: async (workspaceId) => {
        ...
        await lspService.closeWorkspace(workspaceId);
      },
    },
  ...
  const observerSurfacesIpcHandlers = registerObserverSurfacesIpcHandlers({
    ipcMain,
  ...
  const editorBridgeIpcHandlers = registerEditorBridgeIpcHandlers({
    ipcMain,
    mainWindow,
...
interface DefaultElectronAppServicesOptions {
  ...
  observerSurfacesIpcHandlers: ObserverSurfacesIpcHandlers;
...
class DefaultElectronAppServices implements ElectronAppServices {
  ...
  private readonly observerSurfacesIpcHandlers: ObserverSurfacesIpcHandlers;
  private readonly editorBridgeIpcHandlers: EditorBridgeIpcHandlers;
  ...
  public constructor(options: DefaultElectronAppServicesOptions) {
    ...
    this.observerSurfacesIpcHandlers = options.observerSurfacesIpcHandlers;
    this.editorBridgeIpcHandlers = options.editorBridgeIpcHandlers;
  }
