...
import type { E4EditorEvent } from "../../../shared/src/contracts/e4-editor";
import type { WorkspaceId, WorkspaceRegistry } from "../../../shared/src/contracts/workspace";
import {
  E4EditorFileService,
  mapPorcelainStatusToGitBadge,
  parseGitStatusBadges,
  type E4EditorExecFile,
  type E4EditorWatchFactory,
} from "./e4-editor-file-service";

const tempDirs: string[] = [];
const workspaceId = "ws_e4_service" as WorkspaceId;
...
describe("E4EditorFileService", () => {
  ...
  test("scans a sorted file tree with git badges and ignored heavy directories", async () => {
    ...
    expect(result.type).toBe("e4/file-tree/read/result");
  ...
  test("creates, reads, writes, renames, and deletes editor files", async () => {
    ...
    expect(observedEvents.filter((event) => event.type === "e4/file/watch")).toEqual([
      {
        type: "e4/file/watch",
        workspaceId,
        path: "src/new.ts",
  ...
  test("maps git porcelain status to E4 badge states and returns clean requested paths", async () => {
  ...
  test("subscribes to watch events and stops delivering after listener disposal", async () => {
    ...
    const watchFactory: E4EditorWatchFactory = (_root, options, listener) => {
    ...
    await service.readFileTree({ type: "e4/file-tree/read", workspaceId });
    watchListener?.("change", "src/index.ts");
    await waitFor(() => {
      expect(observedEvents.filter((event) => event.type === "e4/file/watch")).toHaveLength(1);
    });
...
async function createTempWorkspace(): Promise<string> {
  const workspaceRoot = await mkdtemp(path.join(os.tmpdir(), "nexus-e4-service-"));
...
function createService(
  ...
  return new E4EditorFileService({
