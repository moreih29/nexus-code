...
const SMOKE_HTML_PATH = "/test/system/file-tree-click-resilience-runtime.fixture.html";
const SMOKE_RESULT_GLOBAL = "__nexusFileTreeClickResilienceSmokeResult";
...
describe("file tree click resilience runtime smoke", () => {
  test("keeps external-to-tree pointer clicks selecting the clicked row and restores prior selection on Tab-entry focus", async () => {
    ...
    expect(output.rendererResult?.iterations).toBe(CLICK_ITERATIONS);
    expect(output.rendererResult?.selectionMatches).toBe(CLICK_ITERATIONS);
    expect(output.rendererResult?.selectionChecks.every((check) => check.matched)).toBe(true);
    expect(output.rendererResult?.tabEntryCheck?.restored).toBe(true);
    expect(output.rendererResult?.openedPaths).toHaveLength(CLICK_ITERATIONS);
  }, SMOKE_TIMEOUT_MS + 10_000);
});

async function runElectronSmoke(smokeUrl: string): Promise<ElectronSmokeOutput> {
  const child = spawn(String(electronBinary), [RUNNER_PATH, smokeUrl, String(SMOKE_TIMEOUT_MS), SMOKE_RESULT_GLOBAL], {
    cwd: APP_ROOT,
    env: {
      ...process.env,
