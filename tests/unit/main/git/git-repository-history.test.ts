...
import type { GitLogArgs } from "../../../../src/main/features/git/domain/git-repository";
import type { GitRepository } from "../../../../src/main/features/git/domain/git-repository";
...
describe("GitRepository history", () => {
  realGitTest("paginates by last-SHA seed so inserted commits do not shift page 2", async () => {
    ...
    try {
      ...
      const repo = newLocalGitRepository("ws-history-page", root, path.join(root, ".git"), gitOnPath!);
  ...
  realGitTest("merge commit detail reports first-parent file entries", async () => {
    ...
    try {
      ...
      const repo = newLocalGitRepository("ws-history-merge", root, path.join(root, ".git"), gitOnPath!);
