...
2. **VS Code 코드 포크가 아니다.** 엔지니어링 의미에서는 VSCode 코드를 포크하지 않고 Monaco와 표준 라이브러리를 사용한다. 포지셔닝 의미에서는 능력 표면—편집·LSP·레이아웃·키바인딩—이 VSCode 수준을 목표로 한다. 차별화는 토큰·타이포·밀도·강조색·AI 하네스 관찰 통합·멀티 워크스페이스 모델에서 만든다.
...
Editor pane 내부 split은 1-depth horizontal split만 지원한다. 좌/우 두 pane과 pane별 탭바까지가 MVP 범위이며, multi-depth split과 grid layout은 v0.2 결정 영역으로 미룬다.
