...
첫 가시 UI 표면은 일반 탭 표식이 아니라 WorkspaceSidebar의 상태 뱃지다. 상태는 4상태 입력을 3개의 시각 출력으로 접는다. completed는 뱃지를 표시하지 않으며 mental model은 "끝났다=조용해졌다"로 고정한다. 뱃지는 텍스트 없는 static 8px dot이며 상태명은 sr-only 텍스트로 제공한다.
