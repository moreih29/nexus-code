...
supersedes: design.md v2 (Bounded Zones / hairline 모델 — Islands 모델로 전면 대체)
design_basis: JetBrains "Islands" theme (2025) 외부 조사 반영
---
...
```
primitive.ts       원시값·불변 (색·스페이싱·라디우스·폰트)
...
### 5축: 톤 — near-monochromatic 저채도

- 뉴트럴 계열은 브랜드 hue로 미세 틴트한다
- 채도 C ≤ 0.012 (near-monochromatic 저채도)
- 순수 `#000` / `#fff` 금지 — 항상 hue가 틴트된 뉴트럴을 쓴다
- warmth는 불변이 아니다. "Warm Dark" 기본 테마의 hue 선택일 뿐이며, 테마별로 가변한다
...
규칙:
- 위 스케일에 없는 임의 px(5, 10, 14, 15, 18px 등)는 in-app UI에 사용할 수 없다.
- `6`은 Islands gap 전용으로 그리드에 정식 편입된 스텝이다. 일반 레이아웃에서도 사용 가능하나
  주 용도는 island gap이다.
...
| 상수 | 기본 | compact | 의미 |
|---|---|---|---|
| island gap | 6px | 4px | 인접한 섬 사이 캔버스 노출 간격 |
| island radius | 10px | 8px | 섬·Floating 표면 모서리 (§4) |
| 버튼 높이 | 28px | 24px | — |
| 버튼 최소 너비 | 72px | 72px | — |
| 입력 필드 높이 | 28px | 24px | — |
...
| 포커스 베일 | 비활성 섬을 흐리게 (아래) |

### 포커스 베일 (Inactive Veil)

포커스를 잃은 섬에는 반투명 veil(`surface.island.inactive.veil`)이 합성되어 섬 전체가 흐려진다.
활성(포커스된) 섬만 완전한 선명도를 갖는다. 이것이 그림자 없이 "지금 활성 섬"을 표현하는 핵심 기법이다.
...
- 베일은 *섬 단위*로만 적용한다. 섬 내부 개별 컴포넌트에는 §8의 상태 토큰을 쓴다.
...
다음은 **색이 본질적 의미**를 전달하는 통제된 예외로, §1 5축의 채도 제약과 redundant encoding 원칙의
예외를 허용한다. 단 보조 인코딩 또는 절제된 채도를 동반해야 한다:
- diff: 추가(녹색 계열) + `+` 기호 / 삭제(적색 계열) + `-` 기호
- git graph: commit 노드 = 원형, merge 노드 = 마름모
- syntax highlighting: 코드 토큰 변별을 위한 hue 사용 (§15). UI chrome의 C ≤ 0.012 제약을
  넘되, 가독성에 필요한 최소 채도로 제한하고 WCAG 본문 대비 4.5:1을 유지한다.
...
themes/warm-dark.ts, cool-dark.ts, warm-light.ts …
  SemanticKey 전체 충족. primitive 값을 참조해 채움
...
|---|---|---|---|---|
| warm-dark | themes/warm-dark.ts | 웜 옐로~황록 계열 | 다크 | 기본·플래그십 |
| cool-dark | themes/cool-dark.ts | 쿨 블루~시안 계열 | 다크 | 명시 선택 |
| warm-light | themes/warm-light.ts | 웜 옐로~황록, warm-dark와 동일 hue 패밀리 | 라이트 | OS Auto 페어 |

OS Auto 페어: warm-dark ⇄ warm-light. cool-dark는 명시 선택 시 OS 추종 해제·고정.
...
채도 예외: 신택스 토큰은 §8 "색-의미 통제 예외"에 속한다. §1 5축의 C ≤ 0.012 near-monochromatic
