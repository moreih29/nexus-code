...
Plan #6 I4에서 Tauri POC(`.nexus/memory/tauri-poc-report.md`) 결과 blocker 0건 확인. Electron은 잠정 유지. 아래 4개 조건 중 2개 이상 충족 시 새 plan 세션으로 Tauri 전환 재평가를 시작한다.

| # | Trigger |
|---|---------|
| T1 | 모바일 배포 요구 발생 |
| T2 | 번들 크기 또는 메모리가 사용자(민지) 실불만으로 가시화 |
| T3 | Phase 3 공식 착수 — OpenCode 1급 구현 시점 |
| T4 | Rust toolchain 정비된 전용 측정 환경 확보 — Track A 미완 지표(30분+ SSE 안정성, WKWebView 250바이트 버퍼링 체감, approval 왕복 ms, macOS .app 전체 번들, Linux webkit2gtk, Windows .msi) 재측정 가능 조건 |

T3(OpenCode 1급)과 T4(전용 빌드 환경)를 동시에 충족하는 시점이 가장 자연스러운 전환 타이밍이다. 이 두 조건이 함께 갖춰지면 `electron` 패키지만 교체하면 되며, Node sidecar(I1-I3 구현)는 재작업 없이 그대로 사용할 수 있다.

상세 실측 수치: `.nexus/memory/tauri-poc-report.md`
