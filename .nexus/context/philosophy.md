...
- **헤드리스 서버 모드** — 3계층 아키텍처가 이미 가능케 함. Mac Studio/홈서버에 server만 띄우고 다른 기기 브라우저 접속. 보안/인증은 S3 안건에서 구체화. 운영 로그도 서버 측 단일 base(`~/.nexus-code/logs/`)에 통합되어 헤드리스 배치에서 자연 동작 — 로그 수집 도구를 서버 머신 한 곳에서 관리.
...
**3. Existing seam 원칙**
현재 4 패키지(shared/server/web/electron) · 7 adapter 카테고리(claude-code/approval/security/hooks/db/events/logging) · 11 route에 자연스럽게 들어가면 OK. **새 adapter 카테고리 필요 = 빨간 불**. 신규 경계는 재검토 대상.
...
## Tauri 런타임 재평가 Trigger [ARCHIVED 2026-04-13 Plan #5 Issue #1]

> **[2026-04-13 Plan #5 supersede]** 이 trigger 표는 Tauri 전면 전환 옵션 A 결정(Issue #1)으로 무효화되었다. 상세 갱신은 별도 plan 세션(Tauri 전환 단계화)에서 진행. 이력 보존 목적으로 표 본체는 그대로 유지.

Plan #6 I4에서 Tauri POC(`.nexus/memory/tauri-poc-report.md`) 결과 blocker 0건 확인. 2026-04-13 후속 교정으로 UI ↔ sidecar ↔ CC 실 왕복(approval full roundtrip 포함) 재현까지 완료(부록 B.4). Electron은 잠정 유지. 아래 4개 조건 중 2개 이상 충족 시 새 plan 세션으로 Tauri 전환 재평가를 시작한다.

| # | Trigger | 현 상태 |
|---|---------|---------|
| T1 | 모바일 배포 요구 발생 | 미충족 |
| T2 | 번들 크기 또는 메모리가 사용자(민지) 실불만으로 가시화 | 미충족 |
| T3 | Phase 3 공식 착수 — OpenCode 1급 구현 시점 | 미충족 |
| T4 | Rust toolchain 정비된 전용 측정 환경 확보 — Track A 미완 지표(30분+ SSE 안정성, WKWebView 250바이트 버퍼링 체감, approval 왕복 ms, macOS .app 전체 번들, Linux webkit2gtk, Windows .msi) 재측정 가능 조건 | 부분 충족 — dev 모드 수준의 실 왕복은 재현됐고, `.app` 번들 산출 및 장시간·크로스플랫폼 지표만 미측정 |

본 결정으로 trigger 기반 재평가 구조는 종료. Phase 1/2 실행 계획은 `.nexus/memory/tauri-migration-plan.md` 참조.

"sidecar Bun 호환성"과 "WKWebView SSE 실동작"이라는 두 핵심 불확실성이 2026-04-13 후속 교정으로 해소되어, T4의 실질 난이도는 "Rust `tauri::generate_handler![]` proc macro 패닉 해소 → `.app` 번들 산출" 한 건으로 축소됐다. T3(OpenCode 1급)와 T4를 동시에 충족하는 시점이 가장 자연스러운 전환 타이밍이라는 판단은 유지 — 전환 시 `electron` 패키지만 교체하면 되고, Node sidecar(I1-I3 구현)는 재작업 없이 그대로 사용 가능.

상세 실측 수치 + 후속 교정 이력: `.nexus/memory/tauri-poc-report.md`
