...
Nexus Code는 **에이전트 감독자를 위한 통합 워크벤치**이며, Nexus 생태계의 **Supervision layer**다. Claude Code(현재)와 OpenCode(계획) 세션을 외부에서 spawn·관찰하고, 권한 요청을 **Policy Enforcement Point**로 중재한다. 내부는 네 개의 패키지(shared/server/web/tauri)로 구성되며, 각 패키지는 단일 책임을 가진다. Tauri가 데스크톱 셸이며, 이전 Electron 셸은 Phase 2 Cycle C에서 폐기되었다.
...
Phase 2 Cycle A/B/C 완료: tauri 2.9.5 + macOS `.app` 번들 산출 + pino-pretty 번들 제외 + dev orchestrator [tauri] 전환 + `@nexus/electron` 폐기. 상세 이력은 `.nexus/memory/tauri-migration-plan.md` 및 cycle-a/b/c-report.md 참조.
...
| Tauri → Web | Tauri command (invoke) | 네이티브 기능(폴더 선택 `select_folder`, 포트 알림 `await_initialization`) |
| Web → Server (dev only) | HTTP POST (`/api/dev/client-log`) | 클라이언트 console 로그 수집 |

## 설계 원칙

1. **단방향 의존**: shared ← server ← web ← tauri. 역방향 참조 금지.
