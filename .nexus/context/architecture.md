...
Nexus Code는 **에이전트 감독자를 위한 통합 워크벤치**이며, Nexus 생태계의 **Supervision layer**다. Claude Code(현재)와 OpenCode(계획) 세션을 외부에서 spawn·관찰하고, 권한 요청을 **Policy Enforcement Point**로 중재한다. 내부는 다섯 개의 패키지(shared/server/web/tauri/electron)로 구성되며, 각 패키지는 단일 책임을 가진다. electron은 Phase 2에서 폐기 예정이며, tauri가 데스크톱 셸을 대체한다.
...
nexus-code(Supervision layer)의 내부는 5개 패키지로 구성된다.

```
┌─────────────────────────────────────────┐
│  Tauri (desktop shell)                  │  Rust 셸 + WebView + sidecar
├─────────────────────────────────────────┤
│  Electron (desktop shell) [deprecated]  │  프로세스 관리 + 창 — Phase 2 폐기 예정
├─────────────────────────────────────────┤
│  Web (React)                            │  UI + 상태
├─────────────────────────────────────────┤
...
### `@nexus/tauri` — 데스크톱 셸 (신규)

Tauri 2.x 기반 데스크톱 셸. `packages/tauri/src-tauri/`에 Rust workspace가 위치하며, Hono sidecar를 `bun compile`로 단일 바이너리로 묶어 Tauri sidecar로 번들링한다. WebView는 native `EventSource`로 localhost SSE를 직접 구독한다(`plugin-http` 우회 — POC 부록 B.1 검증). IPC는 최소화하며 `tauri-plugin-dialog`로 폴더 선택을 대체한다. OpenCode `sst/opencode` 패턴을 참조 구현으로 삼는다.

Phase 1(본 cycle): `packages/tauri/` 파일 scaffolding만 완료. `cargo build` 및 `.app` 번들 산출, `@nexus/electron` 폐기는 Phase 2(별도 cycle)에서 진행한다. 상세 실행 계획은 `.nexus/memory/tauri-migration-plan.md` 참조.

### `@nexus/electron` — 데스크톱 셸

> **[Deprecated — Phase 2에서 폐기 예정]** 자세한 경로는 `.nexus/memory/tauri-migration-plan.md` 참조.
...
빌드 순서: `shared → server/web → tauri` (Phase 2 완료 후 확정; electron은 Phase 2 폐기 예정).

Phase 1 기간 중 실행 경로는 electron을 여전히 spawn하는 상태다 — `scripts/dev.ts`가 Electron을 기동하며, tauri dev 전환(`bun --filter @nexus/tauri dev`)은 Phase 2 dev orchestrator 재설계 시 이루어진다.

1. `@nexus/shared` 빌드 (타입이 먼저 준비돼야 나머지가 컴파일됨)
2. `@nexus/server` + `@nexus/web` dev 서버 병렬 기동
3. `/api/health` + 웹 데브서버(5173)가 준비될 때까지 대기
4. Electron 런치 (Phase 1 현재 경로 — Phase 2에서 `tauri dev`로 전환 예정)
