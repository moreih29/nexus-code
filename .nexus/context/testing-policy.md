...
따라서 이번 정책은 문서와 PR template으로 강제한다. service 디렉터리와 contract test naming이 안정된 뒤, 변경 파일과 test 파일의 매핑을 결정적으로 판정할 수 있는 script/CI gate가 생기면 ESLint가 아니라 전용 check script로 재평가한다.
