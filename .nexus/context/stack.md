...
**패키징**: electron-builder. macOS codesign과 notarization을 포함한다. 현재 E2 기준 `extraResources`에는 node-pty 네이티브 바이너리(`pty.node`)와 OFL 폰트 번들이 포함된다.
...
| macOS 패키징(서명/노타라이즈 환경변수 필요) | `cd packages/app && bun run package:mac` |
| Go sidecar 빌드 | `cd sidecar && go build` |
...
- macOS codesign + notarization — 현재 node-pty·폰트 번들 기준 검증이 필요하며, Go sidecar 바이너리 포함 시 동일 파이프라인을 다시 검증해야 한다.
