...
| # | 작업 | 선결 요건 |
|---|------|-----------|
| 1 | Rust `tauri::generate_handler![]` proc macro 패닉 해소 | Cargo.toml 버전 핀 조정(OpenCode `packages/desktop/src-tauri/Cargo.toml` 참조) 또는 `cargo update` |
| 2 | macOS `.app` 번들 산출 | Rust toolchain + `cargo build` 성공 |
...
| 5 | 부록 B 함정 Phase 2 5건 (sidecar 재빌드 파이프라인, UI-sidecar 계약 정렬, text_delta/text_chunk 중복, sidecar 종료 Rust 핸들러, pino-pretty 번들 제외) | |
...
- [ ] `Cargo.toml`에 tauri 2.x 버전 핀 (OpenCode `packages/desktop/src-tauri/Cargo.toml` 참조)
- [ ] Rust 1.93.1 vs 1.92.x 다운그레이드 필요성 확인 (proc macro 패닉 회피)
...
1. `cargo check` / `cargo build` 통과
2. `bun --filter @nexus/tauri dev` 실행 (Tauri dev mode 창 띄움)
3. `bun --filter @nexus/server compile` 실행 후 `binaries/nexus-sidecar-<triple>` 생성 확인
4. 30분+ SSE 장시간 안정성
5. macOS `.app` 번들 실제 실행 + approval full roundtrip
...
- **Cycle A**: Rust 툴체인 정비 (proc macro 해소 + `.app` 번들)
- **Cycle B**: dev orchestrator tauri dev 전환 + 부록 B 함정 5건 적용
- **Cycle C**: `packages/electron` 폐기 + `dev-smoke-protocol.md` 갱신
- **Cycle D**: 크로스플랫폼 빌드 검증 (Linux/Windows) + 장시간 SSE

---

*문서 버전: Plan #6 task #1, 2026-04-13. Phase 1 완료 후 Phase 2 진입 시 체크리스트 항목을 업데이트한다.*
