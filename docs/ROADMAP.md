...
|------|------|
| 멀티탭 | 여러 세션 동시 작업 |
| 체크포인트 다중 시점 | 현재 세션당 1개. 턴마다 체크포인트 → 타임라인 복원 |
| 체크포인트 복원 맥락 표시 | 시간만으로는 부족. 커밋 메시지/변경 파일 요약 등 "어떤 상태로 돌아갔는지" 맥락 필요. 복원 후 이전 대화 처리 방안도 재논의 | M5 테스트 피드백 |
| Co-Planning 뷰 | TodoWrite → Plan 뷰 + NexusPanel 연계 |
| 파일 첨부 | 채팅에 이미지/파일 드래그앤드롭 |
| 반응형 레이아웃 | 화면 크기에 따라 우측 패널 탭 잘림. 사이드바/패널 동적 축소/숨김 처리 필요 | M5 테스트 피드백 |
| 알림 토글 버그 조사 | 설정에서 알림 off 해도 알림 발생. cmux 환경 vs 독립 앱 환경 구분 필요. settings 값이 main process로 전달되는지 확인 | M5 테스트 피드백 |
