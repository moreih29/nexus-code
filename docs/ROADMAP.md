...
Phase 기반으로 완료된 작업을 묶어 정리하고, 다음 단계 방향을 명확히 한다.

---

### Phase 1: 기반 (완료) — M0 + M1 + M1.5

프로젝트 셋업, MVP, 통신 안정화, 코드 구조 정비.
...
### Phase 2: UI/UX (완료) — M2a + M2b + M3 + M4

시각 개선, 인터랙션 기초, 파일 변경 안전성, 일상 편의.
...
#### M3: 파일 변경 안전성 (퍼미션 enforcement 제외)
...
| 기능 | 설명 | 상태 |
|------|------|------|
| ~~멀티탭 (Phase 1+2)~~ | 구현 후 제거 — 같은 워크스페이스 내 멀티세션이 사용자 니즈(워크스페이스 간 전환)와 불일치. IPC 라우팅 복잡도 + 버그 다발로 revert | 제거됨 |
...
| 기능 | 설명 |
|------|------|
| flex 레이아웃 구조 재검토 | min-width/overflow 정책, 패널 고정 너비 보장. 우측 패널이 채팅 콘텐츠에 밀려 안 보이는 문제 해결 |
| 리사이저블 패널 | 드래그로 사이드바/우측패널 너비 조절 |
| 반응형 전환 최적화 | CSS transition 버벅임 개선 |

**후속 과제:**
- 사이드바 접기/펼치기 시 워크스페이스 영역 버벅임 + 로딩 느림 — 패널 resize 시 불필요한 리렌더링 최적화 필요
...
| 기능 | 설명 |
|------|------|
| 체크포인트 되돌리기 UX 보완 | CLI 컨텍스트 잔존 문제 조사 + 실사용 피드백 기반 개선 |
| 비용 추적 대시보드 | lastTurnStats 기반 누적 통계, 세션/일별 비용 표시 |

---

### Phase 6: 워크스페이스 세션 전환

| 기능 | 설명 |
|------|------|
| IPC 라우팅 구조 조사 | 현재 단일 세션 IPC를 워크스페이스별 동시 세션으로 확장 가능성 검토 |
| 워크스페이스 간 전환 구현 | A 워크스페이스 작업 중 B로 전환, 세션 유지하며 번갈아 작업 |
...
- 간단 에디터 (기본 코드 편집 기능)
- 크로미움 브라우저 내장 (Electron webview 기반 브라우저)
