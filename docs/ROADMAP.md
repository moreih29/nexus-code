...
| 기능 | 설명 |
|------|------|
| Edit/Write diff PermissionCard | manual 모드에서 파일 변경 전 diff 검토 화면 |
| RightPanel "Changes" 탭 | 턴 내 파일 변경 집계 뷰 |
| 승인 범위 확장 | 이번 한 번 / 세션 / 워크스페이스 / 영구 4단계 |
| 체크포인트 | git stash 기반. 세션 시작 시 스냅샷 자동 생성 + Restore 버튼 |

**완료 기준:**
- diff 뷰 표시 동작 확인
- 체크포인트 생성 및 복원 동작 확인
- 승인 범위 3단계 이상 선택 UI 확인

---

### M4: 에이전트 경험 + 편의

에이전트 타임라인 고도화와 편의 기능을 추가한다.

| 기능 | 설명 |
|------|------|
| AgentTimeline 이벤트 필터/확장 | 유형 필터, 타임스탬프, LLM 호출 로그 |
| 커맨드 팔레트 (CMD-K) | 새 세션, 설정, 히스토리 빠른 접근 |
...
M1~M4 진행 후 재평가하여 우선순위를 결정한다.

**M3 후속 — 퍼미션 enforcement + 체크포인트 고도화:**
...
| 체크포인트 복원 후 메시지 처리 | 복원 시 이후 대화 삭제 여부 논의 필요. 삭제하면 깔끔하나 정보 유실. 유지하면 혼란. 절충안: 시각적 구분선 + "이 시점으로 되돌림" 마커 | 심층 논의 필요 — 되돌린 이유가 "잘못된 수정"이면 이후 대화도 무의미할 수 있고, "실험적 시도"면 대화 기록은 유지하고 싶을 수 있음 |
| 체크포인트 다중 시점 | 현재 세션당 1개. 턴마다 체크포인트 → 타임라인 복원 | 복잡도 높음, 후순위 |
...
리서치 과정에서 식별된 3가지 근본 제약. 기능 설계 시 반드시 고려해야 한다.
...
### 제약 2: tool_call 이벤트는 실행 후 도착

Edit/Write 실행 전 승인/거부는 HookServer manual 모드에서만 가능하다. auto 모드에서는 파일이 이미 수정된 후 알림이 도착한다. 따라서 파일 변경 사전 검토는 반드시 HookServer manual 모드를 전제로 설계해야 한다.
