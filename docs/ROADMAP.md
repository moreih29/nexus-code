...
**후속 과제:**
- 워크스페이스 간 세션 전환 조사 — A 워크스페이스 작업 시켜놓고 B로 전환, 세션 유지하며 번갈아 작업. IPC 라우팅 구조(현재 단일 세션)를 워크스페이스별 세션 동시 유지로 확장 가능한지 조사 필요
- 레이아웃 리팩토링 — 우측 패널이 채팅 콘텐츠에 의해 밀려서 안 보이는 문제. 현재 반응형(900px 브레이크포인트)만으로는 부족, flex 레이아웃 구조 자체 재검토 필요 (min-width/overflow 정책, 패널 고정 너비 보장 등)
