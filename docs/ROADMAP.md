...
| 기능 | 설명 | 비고 |
|------|------|------|
| 알림 토글 버그 조사 | 설정에서 알림 off 해도 알림 발생. 독립 앱 빌드 후 재현 확인 → cmux 이슈면 문서화, 코드 버그면 수정 | 조사+판별만 |
| 체크포인트 복원 맥락 표시 | 구분선에 커밋 해시/변경 파일 요약 표시. 복원 이후 메시지는 UI에서 삭제 + 자동 안내 프롬프트 전송 ("체크포인트 복원됨, 이후 변경 무효화") | C안 채택 |
| 반응형 레이아웃 | 브레이크포인트 기반 자동 숨김. `<900px` 우측 패널 접힘, `<700px` 사이드바 오버레이. ResizeObserver 또는 미디어 쿼리 | A안, 리사이저블은 Tier 3 |
| 파일 첨부 | 채팅에 이미지/파일 드래그앤드롭 | |

**완료 기준:**
- 체크포인트 복원 시 맥락 라벨 + 이후 대화 삭제 확인
- 화면 900px 이하에서 우측 패널 자동 접힘 확인
- 알림 버그 원인 판별 완료
