...
`.dmg`를 마운트하고 **Nexus Code**를 `/Applications`로 드래그한다.

### 3. Gatekeeper 우회

Nexus Code는 현재 Apple 공증(notarization) 없이 배포된다. 처음 실행 시 macOS Gatekeeper가 앱을 차단할 수 있다.

#### macOS 14 (Sonoma)

1. Finder에서 `/Applications/Nexus Code.app`을 우클릭(또는 Control+클릭)한다.
2. 메뉴에서 **Open**을 선택한다.
...
3. 하단 "Nexus Code was blocked…" 항목 옆 **Open Anyway** 버튼을 클릭한다.
...
```bash
xattr -dr com.apple.quarantine "/Applications/Nexus Code.app"
```
